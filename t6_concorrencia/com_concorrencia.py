...
def distributed_lock(r: redis.Redis, resource: str, ttl: int = 5):
	...
	NX: so cria se a chave nao existir (atomico no Redis).
	EX: define TTL para evitar deadlock permanente.
	"""
	key = f"lock:{resource}"
...
def transferir_com_lock(valor: int, nome: str):
	"""Transferencia COM lock distribuido (segura entre processos)."""
	r = get_redis()
